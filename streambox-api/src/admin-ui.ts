...
function toast(m,e){const x=document.getElementById('toast');x.textContent=m;x.classList.remove('hidden');x.style.borderColor=e?'#b84b5d':'#384255';setTimeout(()=>x.classList.add('hidden'),3500)}
...
async function login(){try{await api('/v1/admin/session',{method:'POST',body:JSON.stringify({token:token.value})});token.value='';showApp();await load()}catch(e){toast(e.message,1)}}
async function logout(){try{await fetch('/v1/admin/session',{method:'DELETE',credentials:'same-origin'})}finally{showLogin()}}
function card(p){let f=(fields[p.id]||[]).map(a=>{const k=a[0],lab=a[1],sec=a[2],set=sec&&p.secretConfigured&&p.secretConfigured[k],v=sec?'':((p.publicConfig&&p.publicConfig[k])||'');return '<div class="field"><label>'+lab+(set?' <span class="ok">• configured</span>':'')+'</label><input data-field="'+k+'" type="'+(sec?'password':'text')+'" value="'+v+'" placeholder="'+(sec?(set?'Leave blank to keep existing':'Enter value'):'')+'"></div>'}).join('');return '<div class="card provider" data-provider="'+p.id+'"><div class="provider-head"><div><h3>'+p.name+'</h3><span class="pill">'+p.source+'</span></div><label><input class="enabled" type="checkbox" '+(p.enabled?'checked':'')+'> Enabled</label></div><p class="muted">'+p.purpose+'</p>'+(p.configError?'<p class="warn">'+p.configError+'</p>':'')+'<div class="fields">'+f+'</div><button onclick="saveProvider(\''+p.id+'\')">Save provider</button></div>'}
function renderProviders(p){providers.innerHTML=p.map(card).join('')}
...
function row(v){return '<tr>'+v.map(x=>'<td>'+String(x??'')+'</td>').join('')+'</tr>'}
function render(d){stats.innerHTML=[['Providers enabled',d.stats.providersEnabled+'/'+d.stats.providersTotal],['Streams',d.stats.streams],['Licenses',d.stats.licenses],['Backend','Online']].map(x=>'<div class="card stat"><span class="muted">'+x[0]+'</span><strong>'+x[1]+'</strong></div>').join('');renderProviders(d.providers);streams.innerHTML=d.streams.map(s=>row([s.catalog_id,s.title,s.source_type,s.rights_status,s.territory,s.updated_at])).join('');licenses.innerHTML=d.licenses.map(l=>row([l.catalog_id,l.marketplace,l.reference,l.territory,l.created_at])).join('')}
