...
<div id="loginCard" class="card login"><div class="brand">Stream<span>Box</span> Admin</div><p class="muted">Enter ADMIN_TOKEN. It is exchanged for a secure HttpOnly session.</p><div class="field"><input id="token" type="password" placeholder="ADMIN_TOKEN"></div><div style="margin-top:12px"><button onclick="doLogin()">Login</button></div></div>
...
async function api(p,o={}){const r=await fetch(p,{...o,credentials:'same-origin',headers:{'content-type':'application/json',...(o.headers||{})}});const t=await r.text();let b={};try{b=JSON.parse(t)}catch{b={error:t}}if(r.status===401&&p!=='/v1/admin/session'){showLogin();throw Error('Admin session expired')}if(!r.ok)throw Error(b.error||('Request failed '+r.status));return b}
function showLogin(){el('loginCard').classList.remove('hidden');el('app').classList.add('hidden')}function showApp(){el('loginCard').classList.add('hidden');el('app').classList.remove('hidden')}
async function doLogin(){const tokenEl=el('token');try{const result=await api('/v1/admin/session',{method:'POST',body:JSON.stringify({token:tokenEl.value})});if(!result.ok)throw Error('Login failed');tokenEl.value='';await load();showApp()}catch(e){toast(e.message||'Login failed',1)}}
